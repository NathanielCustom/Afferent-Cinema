...
class SQLDatabase(Database):
    ...
    SELECT_MULTIPLE_FROM_ID = """
        SELECT HEX(%s), %s, %s FROM %s WHERE (%s IN (%%s)) AND (%s = (%%s));
    """ % (Database.FIELD_HASH, Database.FIELD_SONG_ID, Database.FIELD_OFFSET,
    ...
    def return_matches(self, hashes, id_specific):
        """
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                ...
                if id_specific != None:
					print "specified ID"
					query = self.SELECT_MULTIPLE_FROM_ID
					query = query % (', '.join(['UNHEX(%s)'] * len(split_values)), id_specific)
                else:
					print "ID = none"
